...
namespace eng
{
	...
	public:
		...
		LocalTexture2D(const LocalTexture2D& crTexture);
		LocalTexture2D(LocalTexture2D&& rrTexture) noexcept;
		LocalTexture2D& operator=(const LocalTexture2D& crTexture);
		LocalTexture2D& operator=(LocalTexture2D&& rrTexture) noexcept;
		~LocalTexture2D();
		...
		bool SetSubregion(const LocalTexture2D& crTexture, sint32 positionX, sint32 positionY);
		...
		constexpr uint8* GetData() const noexcept { return m_pData; }

		constexpr operator bool() const noexcept { return m_pData != nullptr; }
		constexpr bool operator==(const LocalTexture2D& crTexture) const noexcept { return m_pData == crTexture.m_pData; }
